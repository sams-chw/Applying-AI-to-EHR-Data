...
def check_null_values(df):
    null_df = pd.DataFrame({'columns': df.columns, 
                            ...
                            'count_?': df.isin(['?']).sum(),
                            'count_total': len(df),
                            'percent_nan': np.round(df.isnull().sum()*100/len(df),1),
                            'percent_?': np.round(df.isin(['?']).sum()*100/len(df),1)
                           } )
    null_df.set_index('columns', inplace = True)
    null_df['count_total_invalid'] = null_df.iloc[:, 0:2].sum(axis=1)
    null_df['percent_total_invalid'] = null_df.iloc[:, 3:5].sum(axis=1)
    return null_df

def aggregate_dataset(df, grouping_field_list,  array_field):
    df = df.groupby(grouping_field_list).apply(lambda x: x[array_field].values.tolist()).reset_index()
    df = df.rename(columns={0: array_field + "_array"}) 
